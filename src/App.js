...
function App() {
  ...
  return (
    ...
    <ProgressBar currentQuestionIndex = {score} totalQuestionsCount = {questions.length} color = {"#023020"} />
    <div className="percentage">{(score/questions.length) * 100 }</div>
    <ProgressBar currentQuestionIndex = {wrongAnswer} totalQuestionsCount = 
    {questions.length} color = {"#FF5733"} />
    </div>
    <div className="app">
      {showScore ? (
